...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
    ...
    <xsl:template match="dc:date">
	    <xsl:choose>
		  <xsl:when test="ends-with(text(), 'Z')">
            <!--add other date -->
		  </xsl:when>
		  <xsl:otherwise>
		    <originInfo>
    ...
    <xsl:template match="dc:identifier">  
        <xsl:if test="starts-with(text(), 'http://') or starts-with(text(), 'https://')">
			...
			<xsl:choose>
			  <xsl:when test="contains(text(), 'dc.library.okstate.edu') or contains(text(), 'hdl.handle.net/11244')">
			    <note type="ownership">
    ...
    <xsl:template match="dc:relation">
		...
		<relatedItem type="host">
		  <titleInfo>
		    <title>Oklahoma Cooperative Extension Service</title>
		  </titleInfo>
		</relatedItem>
    </xsl:template>
