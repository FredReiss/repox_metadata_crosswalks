...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
    ...
    <xsl:template match="dc:date">
	  <xsl:choose>
	   <xsl:when test="matches(text(),'^\d+-\d+-\d{2}$')">
	    <xsl:variable name="dukeDate" select="tokenize(., '-')"/>
		
	    <originInfo>
            <dateCreated>
                <xsl:value-of select="concat($dukeDate[1],'-',$dukeDate[2],'-19',$dukeDate[3])"/>
            </dateCreated>
        </originInfo>
	   </xsl:when>
	   <xsl:otherwise>
	...
	<xsl:template match="dc:relation">
	   <xsl:choose>
		<xsl:when test="contains(text(), 'Doris Duke Collection')">
           <relatedItem type="host">
		       <titleInfo><title><xsl:apply-templates/></title></titleInfo>
			   <location>
				  <url>
					https://digital.libraries.ou.edu/cdm/landingpage/collection/dorisduke
			 	  </url>
			    </location>
            </relatedItem>
		</xsl:when>
		<xsl:when test="contains(text(), 'Western History Collections')">
			<relatedItem type="host">
			   <location>
				  <url>
					https://libraries.ou.edu/content/western-history-collections-0
			 	  </url>
			    </location>
            </relatedItem>	   
		</xsl:when>
    ...
    <xsl:template match="dc:rights">
        <accessCondition type="use and reproduction" xlink:href="http://rightsstatements.org/vocab/InC/1.0/"></accessCondition>
		<accessCondition type="local rights statement">
            For attribution information, please visit https://digital.libraries.ou.edu/TOU_WHC2.php
        </accessCondition>
    </xsl:template>
</xsl:stylesheet>
