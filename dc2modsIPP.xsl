...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
    ...
    <xsl:template match="dc:date">
	  <xsl:choose>
	   <xsl:when test="matches(text(),'^\d+-[A-Z][a-z][a-z]-\d{2}$')">
	    <xsl:variable name="ippDate" select="tokenize(., '-')"/>
	...
	<xsl:template match="dc:relation">
        <relatedItem type="host">
		   ...
		   <location>
			  <url>
				https://digital.libraries.ou.edu/cdm/landingpage/collection/indianpp
		 	  </url>
		    </location>
        </relatedItem>
    </xsl:template>
